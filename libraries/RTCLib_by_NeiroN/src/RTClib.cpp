...
DateTime DS1302::now() {
    ...
    read(); // ignore dayOfTheWeek register since we have our own method
...
void DS1307::adjust(const DateTime& dt) {
    ...
    RTCLIB_WIRE.write(bin2bcd(dt.dayOfTheWeek()));
...
DateTime DS1307::now() {
    ...
    RTCLIB_WIRE.read(); // ignore dayOfTheWeek register
...
void PCF8583::adjust(const DateTime& dt) {
    ...
    RTCLIB_WIRE.write((dt.dayOfTheWeek() << 5) | (bin2bcd(dt.month()) & 0x1f));
    RTCLIB_WIRE.endTransmission();
...
void PCF8563::adjust(const DateTime& dt) {
    ...
    RTCLIB_WIRE.write(bin2bcd(dt.dayOfTheWeek())); // Weekday (0-6 = Sunday-Saturday)
...
void PCF8563::set_alarm(const DateTime& dt, alarm_flags flags) {
    ...
    uint8_t dayOfTheWeek = bin2bcd(dt.month());
    ...
    RTCLIB_WIRE.write(dayOfTheWeek);
    RTCLIB_WIRE.endTransmission();
}
