#include "Aisassistant.h"
#include <base64.hpp>  //放在Aisassistant.h里会报错，只能放在这里
#ifndef I2S_OUT_PORT
...
const char *baidu_api_key =  "*";//请填写
const char *baidu_secret_key =  "*";//请填写
...
const char *Spark_authorization = "Bearer ***";//请填写
  ...
  if (httpResponseCode > 0) {
    if (httpResponseCode == HTTP_CODE_OK) {
      ...
      if (contentType.startsWith("audio")) {
        ...
        while (http.connected() && (len > 0 || len == -1)) {  // http.connected() && (bytesRead = stream->readBytes(buffer, sizeof(buffer))
          // 音频输出
          ...
          //    i2s_write(I2S_OUT_PORT, (int16_t*)buffer, bytesRead, &bytesWritten, portMAX_DELAY);
          //    //printf("Written %d bytes to I2S\n", bytesWritten);
          //    //vTaskDelay(9);

          //   // if (bytesWritten < bytesRead) {
          //   //  // printf("Warning: Not all bytes were written to I2S\n");
          //   // }
          //   memset(buffer, 0, sizeof(buffer));
          // }
...
String label = "";
////////////////////Spark/////////////////////////////////////
const char *SparkLLM_Get(const char *requestBody1, String &label_change = label) {
  ...
  // 去除开头的两个字符
  requestBody1_str = requestBody1_str.substring(2);
  // 去除结尾的两个字符
  requestBody1_str = requestBody1_str.substring(0, requestBody1_str.length() - 2);


    String requestBody = requestBody0 + requestBody1_str + requestBody2;
...
const char *STT_GET(const char *access_token, uint8_t *audioData, int audioDataSize) {
  switch (STT_TTS_CHOICE) {
    case 0:
      ...
      String response = baiduSTT_Get(access_token_str, audioData, audioDataSize);
  ...
  while (1) {
    if (chatstate == 0 || chatstate == 1) {
      ...
      if (recordingSize > 0) {
        ...
        else if (LLM_CHOICE == 1) LLMResponse = SparkLLM_Get(recognizedText.c_str(), label);

        // 星火大模型
        //  String ernieResponse  = getXunFeiAnswer(recognizedText);
        Serial.println("LLMResponse: " + LLMResponse);
        // SoundSetup();
        //  文本转音频tts并通过MAX98357A输出（语音合成API访问）
        TTS_GET(baidu_access_token.c_str(), LLMResponse.c_str());
