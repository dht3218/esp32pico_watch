...
APDS9960::APDS9960(TwoWire &wire, int intPin) : _wire(wire),
                                                _intPin(intPin),
...
bool APDS9960::begin()
{
  ...
  if (!setWTIME(0xFF))
    return false;
  if (!setGPULSE(0x43))
    return false; // 16us, 16 pulses // default is: 0x40 = 8us, 1 pulse
  if (!setPPULSE(0x03))
    return false; // 16us, 16 pulses // default is: 0x40 = 8us, 1 pulse
  if (!setGestureIntEnable(true))
    return false;
  if (!setGestureMode(true))
    return false;
  if (!enablePower())
    return false;
  if (!enableWait())
    ...
    for (int i = 0; i + 3 < bytes_read; i += 4)
    {
      ...
      if (u < gestureThreshold && d < gestureThreshold && l < gestureThreshold && r < gestureThreshold)
      {
        ...
        if (_gestureDirInX != 0 || _gestureDirInY != 0)
        {
          ...
          // else if (totalY > _gestureSensitivity) { _detectedGesture = GESTURE_UP; }
          // else if (totalX < -_gestureSensitivity) { _detectedGesture = GESTURE_LEFT; }
          // else if (totalX > _gestureSensitivity) { _detectedGesture = GESTURE_RIGHT; }
...
int APDS9960::gestureAvailable()
{
  ...
  {
    // Serial.println("gestureFIFOAvailable"+gestureFIFOAvailable());
    return 0;
  }
...
//   if (gestureFIFOAvailable() <= 0)
//   {
//     Serial.println("noges");
//     return 0;
//   }

//   handleGesture();
//   if (_proximityEnabled)
//   { Serial.println("_proximityEnabled");
//     setGestureMode(false);
//   }
//   return (_detectedGesture == GESTURE_NONE) ? 0 : 1;
// }
